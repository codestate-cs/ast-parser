...
describe('FileUtils', () => {
  ...
  describe('path utilities', () => {
    ...
    it('should resolve paths', () => {
      expect(FileUtils.resolve('path', 'to', 'file.ts')).toBe(
        path.resolve('path', 'to', 'file.ts')
      );
    });

    it('should normalize paths', () => {
      expect(FileUtils.normalize('/path/to/../file.ts')).toBe(
        path.normalize('/path/to/../file.ts')
      );
    });

    it('should get relative paths', () => {
      expect(FileUtils.relative('/path/to', '/path/to/file.ts')).toBe(
        path.relative('/path/to', '/path/to/file.ts')
      );
    });
  });
  ...
  describe('writeFile advanced options', () => {
    ...
    it('should throw error when file exists and overwrite is false', async () => {
      ...
      await expect(FileUtils.writeFile(filePath, content, { overwrite: false })).rejects.toThrow(
