...
export class APITemplate extends BaseTemplate {
  ...
  public override validateConfiguration(
    config: Partial<APITemplateOptions> = this.options
  ): ConfigurationValidationResult {
    const errors: string[] = [];
    ...
    if (
      this.options.includeMethodSignatures !== undefined &&
      typeof this.options.includeMethodSignatures !== 'boolean'
    ) {
      errors.push('includeMethodSignatures must be a boolean');
    }

    if (
      this.options.includeTypeDefinitions !== undefined &&
      typeof this.options.includeTypeDefinitions !== 'boolean'
    ) {
      errors.push('includeTypeDefinitions must be a boolean');
    }

    if (
      this.options.includeParameterDescriptions !== undefined &&
      typeof this.options.includeParameterDescriptions !== 'boolean'
    ) {
      errors.push('includeParameterDescriptions must be a boolean');
    }

    if (
      this.options.includeReturnTypes !== undefined &&
      typeof this.options.includeReturnTypes !== 'boolean'
    ) {
    ...
    if (
      ...
      !['simple', 'detailed', 'comprehensive'].includes(this.options.apiStyle)
    ) {
      errors.push('apiStyle must be one of: simple, detailed, comprehensive');
    }

    if (
      this.options.maxMethodsDisplay !== undefined &&
      (typeof this.options.maxMethodsDisplay !== 'number' || this.options.maxMethodsDisplay < 0)
    ) {
